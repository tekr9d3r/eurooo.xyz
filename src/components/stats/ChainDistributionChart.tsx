...
export function ChainDistributionChart({ chains, isLoading }: ChainDistributionChartProps) {
  ...
  // Top 7 chains, group rest as "Other"
  const top7 = chains.slice(0, 7);
  const others = chains.slice(7);
  
  const chartData = top7.map((c) => ({
    name: c.chain,
    value: c.supply,
  }));

  if (others.length > 0) {
    const otherTotal = others.reduce((sum, c) => sum + c.supply, 0);
