...
export type SummerDepositStep = 'idle' | 'checking' | 'approving' | 'waitingApproval' | 'depositing' | 'waitingDeposit' | 'success' | 'error';

export function useSummerDeposit() {
  const { address } = useAccount();
  const chainId = useChainId();
  const [step, setStep] = useState<SummerDepositStep>('idle');
  ...
  const deposit = useCallback(async (amount: number) => {
    ...
    try {
      ...
      console.error('Summer deposit error:', e);
